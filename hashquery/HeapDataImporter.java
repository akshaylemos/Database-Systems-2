...
public class HeapDataImporter {
    ...
    public HeapDataImporter(int pagesize, int recordSize) {
    ...
    public Page readPageFromFile(int pageNumber) {
        Page page = new Page((pagesize/recordSize));
        byte[] data = new byte[pagesize];
        try {
            //skip bytes to get expected data
            in.skipBytes(pageNumber*pagesize);
            ...
            for (int i = 0; i < recordCount; i++) {
                ...
                page.insertRecord(parseRecord(recordData));
            }
            //initialize the inputStream for next search
            in = new DataInputStream(new FileInputStream("heap." + Integer.toString(pagesize)));
        } catch (IOException e) {
